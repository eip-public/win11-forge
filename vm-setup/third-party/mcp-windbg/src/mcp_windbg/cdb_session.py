...
class CDBSession:
    def __init__(
        ...
        local_attach: Optional[str] = None,   # win11-forge addition: "pid:1234" or "name:foo.exe"
        ...
        Args:
            ...
            remote_connection: Remote debugging connection string.
            local_attach: Attach to a local live process. Formats accepted:
                          "pid:1234"    -> cdb -p 1234
                          "name:x.exe"  -> cdb -pn x.exe
                          (win11-forge addition)
        ...
        sources = [bool(dump_path), bool(remote_connection), bool(local_attach)]
        if sum(sources) == 0:
            raise ValueError("Provide one of dump_path, remote_connection, or local_attach")
        if sum(sources) > 1:
            raise ValueError("dump_path, remote_connection, and local_attach are mutually exclusive")
        ...
        self.local_attach = local_attach
        ...
        elif self.local_attach:
            # Accept "pid:1234" or "name:foo.exe"; also accept bare digits as pid.
            spec = self.local_attach.strip()
            if spec.startswith("pid:"):
                cmd_args.extend(["-p", spec[4:].strip()])
            elif spec.startswith("name:"):
                cmd_args.extend(["-pn", spec[5:].strip()])
            elif spec.isdigit():
                cmd_args.extend(["-p", spec])
            else:
                cmd_args.extend(["-pn", spec])
        ...
        try:
            # Create a new process group for anything where CTRL+BREAK is needed
            # (remote AND local live targets — both are interruptible running processes).
            creationflags = 0
            if os.name == 'nt' and (self.remote_connection or self.local_attach):
