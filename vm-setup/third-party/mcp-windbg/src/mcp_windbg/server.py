...
# win11-forge addition: local live-process attach.
class OpenWindbgLocal(BaseModel):
    """Parameters for attaching to a local live process via cdb -p/-pn."""
    target: str = Field(
        description=("Process to attach to. Formats: 'pid:1234', 'name:foo.exe', "
                     "a bare integer (treated as PID), or a bare process name "
                     "(treated as image name, matches first instance).")
    )
    include_stack_trace: bool = Field(default=False, description="Whether to include stack traces in the initial output")
    include_modules: bool = Field(default=False, description="Whether to include loaded module information")
    include_threads: bool = Field(default=False, description="Whether to include thread information")


class CloseWindbgLocalParams(BaseModel):
    """Parameters for ending a local-attach session."""
    target: str = Field(description="Same target string used in open_windbg_local")
...
class RunWindbgCmdParams(BaseModel):
    ...
    local_target: Optional[str] = Field(default=None, description="Local attach target (win11-forge): 'pid:N', 'name:foo.exe', bare PID, or bare name.")
    command: str = Field(description="WinDbg command to execute")

    @model_validator(mode='after')
    def validate_connection_params(self):
        """Validate that exactly one of dump_path, connection_string, or local_target is provided."""
        sources = [bool(self.dump_path), bool(self.connection_string), bool(self.local_target)]
        if sum(sources) == 0:
            raise ValueError("Provide one of dump_path, connection_string, or local_target")
        if sum(sources) > 1:
            raise ValueError("dump_path, connection_string, and local_target are mutually exclusive")
        return self
...
def _session_id(dump_path: Optional[str],
                connection_string: Optional[str],
                local_target: Optional[str]) -> str:
    if dump_path:
        return os.path.abspath(dump_path)
    if connection_string:
        return f"remote:{connection_string}"
    if local_target:
        return f"local:{local_target}"
    raise ValueError("no session source provided")


def get_or_create_session(
    dump_path: Optional[str] = None,
    connection_string: Optional[str] = None,
    local_target: Optional[str] = None,     # win11-forge addition
    ...
    sources = [bool(dump_path), bool(connection_string), bool(local_target)]
    if sum(sources) == 0:
        raise ValueError("Provide one of dump_path, connection_string, or local_target")
    if sum(sources) > 1:
        raise ValueError("dump_path, connection_string, and local_target are mutually exclusive")

    session_id = _session_id(dump_path, connection_string, local_target)
...
def unload_session(dump_path: Optional[str] = None,
                   connection_string: Optional[str] = None,
                   local_target: Optional[str] = None) -> bool:
...
def _create_server(
    ...
    async def list_tools() -> list[Tool]:
        return [
            ...
            # win11-forge additions: local live-process attach support.
            Tool(
                name="open_windbg_local",
                description=("Attach CDB to a LOCAL live process by PID or image name. "
                             "Accepts 'pid:1234', 'name:foo.exe', a bare integer (PID), "
                             "or a bare image name. CDB halts at attach; use run_windbg_cmd "
                             "with local_target=<same> to send commands."),
                inputSchema=OpenWindbgLocal.model_json_schema(),
            ),
            Tool(
                name="close_windbg_local",
                description="Detach and clean up a local-attach CDB session.",
                inputSchema=CloseWindbgLocalParams.model_json_schema(),
            ),
        ]
    ...
    async def call_tool(name, arguments: dict) -> list[TextContent]:
        try:
            if name == "open_windbg_dump":
                ...
                target = (args.dump_path or
                          (f"remote: {args.connection_string}" if args.connection_string else None) or
                          f"local: {args.local_target}")
                return [TextContent(
                    type="text",
                    text=f"Sent CTRL+BREAK to CDB session ({target})."
                )]

            # win11-forge: open a local live-process debug session.
            elif name == "open_windbg_local":
                args = OpenWindbgLocal(**arguments)
                session = get_or_create_session(
                ...
                results = []
                # Initial register snapshot (CDB halts at entry after attach)
                current_state = session.send_command("r")
                results.append("### Initial Break — Registers\n```\n" + "\n".join(current_state) + "\n```\n\n")
                current_pos = session.send_command("? @$pcurip")
                results.append("### Instruction pointer\n```\n" + "\n".join(current_pos) + "\n```\n\n")

                if args.include_stack_trace:
                    stack = session.send_command("kb 20")
                    results.append("### Stack Trace\n```\n" + "\n".join(stack) + "\n```\n\n")
                if args.include_modules:
                    modules = session.send_command("lm")
                    results.append("### Loaded Modules\n```\n" + "\n".join(modules) + "\n```\n\n")
            ...
            # win11-forge: close a local live-process session.
            elif name == "close_windbg_local":
                args = CloseWindbgLocalParams(**arguments)
                success = unload_session(local_target=args.target)
                return [TextContent(
                    type="text",
                    text=(f"Closed local-attach session for {args.target}"
                          if success
                          else f"No active local-attach session found for {args.target}")
                )]
