...
[[ "$SSH_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]] && ((SSH_TIMEOUT_SECONDS >= 5)) ||
    {
        echo "SSH_TIMEOUT_SECONDS must be a positive integer >= 5 (got: '$SSH_TIMEOUT_SECONDS')" >&2
        exit 1
    }
USE_KEY=false
...
[[ -x "$SETUP_VM" ]] || {
    echo "Missing setup script: $SETUP_VM" >&2
    exit 1
}
...
log "Waiting for SSH"
wait_for_ssh || {
    echo "SSH did not become ready on $VM_IP" >&2
    exit 1
}

log "Waiting for guest tools_ready state"
wait_for_tools_ready || {
    echo "Guest did not report tools_ready on $VM_IP" >&2
    exit 1
}
...
[[ -n "$ACTIVE_DISK" ]] || {
    echo "Could not determine active disk source for $VM_NAME" >&2
    exit 1
}
...
if [[ "$VERIFY_RESTORE" == "true" ]]; then
    ...
    wait_for_ssh || {
        echo "SSH did not return during restore verification" >&2
        exit 1
    }
    log "Waiting for tools_ready during restore verification"
    wait_for_tools_ready || {
        echo "Guest did not report tools_ready during restore verification" >&2
        exit 1
    }
